...
-(void)generateData
{
    if ( plotData == nil ) {
        ...
        _minAltitude = track.altitude;
        _maxAltitude = track.altitude;
        
        NSDate *startDate = track.dateandtime;
        
        NSMutableArray *contentArray = [NSMutableArray arrayWithCapacity:_tracks.count*_tracks.count];
        for ( NSUInteger i = 0; i < _tracks.count; i++ ) {
            Tracking *track = (Tracking *)_tracks[i];
            NSNumber *x = [[NSNumber alloc] initWithDouble:[startDate timeIntervalSinceDate:track.dateandtime]];
            
            if ([track.altitude floatValue] > [_maxAltitude floatValue]) {
                _maxAltitude = track.altitude;
            }
            if ([track.altitude floatValue] < [_minAltitude floatValue]) {
                _minAltitude = track.altitude;
            }
            NSNumber *y = track.altitude;
...
-(void)renderInLayer:(CPTGraphHostingView *)layerHostingView withTheme:(CPTTheme *)theme animated:(BOOL)animated
{
    ...
    y.axisConstraints             = [CPTConstraints constraintWithLowerOffset:50.0];
    ...
    CPTPlotRange *globalYRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat([_minAltitude floatValue])
                                                              length:CPTDecimalFromFloat([_maxAltitude floatValue])];
